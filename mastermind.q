...
pick:{[f;S;G] / (f)unction, (S)et of unused records, logical (G)uesses
 ...
 S:where f S!(group G score\:) each S; / filter all unpicked codes for best split
...
/ algorithms
simple:{(1#key x)!enlist 1b}                   / simple case
...
stdin:{k!k~\:(not in[;k:key[x]]@)guess/""}
...
a:(count game[`simple;S;"0000"]@) peach 50?S
b:(count game[`msize;S;"0011"]@) peach 50?S
dist c:(count game[`esize;S;"0012"]@) peach 50?S
dist d:(count game[`entropy;S;"0123"]@) peach 50?S
dist e:(count game[`mparts;S;"0012"]@) peach 50?S
summary[c] each game[`simple;S;"0011"] c:S 10
summary[c] each game[`msize;S;"0011"] c:S 10
summary[c] each game[`esize;S;"0011"] c:S 10
summary[c] each game[`entropy;S;"0011"] c:S 10
summary[c] each game[`msize;S;"0011"] c:rand S
summary[c] each game[`msize;S;"0011"] c:"0231"
summary[c] each game[`stdin;S;"0011"] c:"0231"
